...
var rowtypedata = [
	[[1, ''], [1,'<input type="text" class="txt" name="newpersno[]" value="" />', 'td25'], [5, '<div><input name="newdepartment_code[]" value="顶级部门编码" size="20" type="text" class="txt" /><input name="newname[]" value="顶级部门名称" size="20" type="text" class="txt" /><input type="hidden" name="newid[]" value="1" /><a href="javascript:;" class="deleterow" onClick="deleterow(this)">删除</a></div>']],
	[[1, ''], [1,'<input type="text" class="txt" name="newpersno[]" value="" />', 'td25'], [5, '<div class="board"><input name="newdepartment_code[]" value="部门编码" size="20" type="text" class="txt" /><input name="newname[]" value="部门名称" size="20" type="text" class="txt" /><input type="hidden" name="newids[]" value="2" /><a href="javascript:;" class="deleterow" onClick="deleterow(this)">删除</a><select name="newinherited[]"><option value="">指定上级部门</option>' + forumselect + '</select></div>']],
	[[1, ''], [1,'<input type="text" class="txt" name="newpersno[]" value="" />', 'td25'], [5, '<div class="board"><input name="newdepartment_code[]" value="部门编码" size="20" type="text" class="txt" /><input name="newname[]" value="部门名称" size="20" type="text" class="txt" /><input type="hidden" name="newids[]" value="2" /><a href="javascript:;" class="deleterow" onClick="deleterow(this)">删除</a><select name="newinherited[]"><option value="">指定上级部门</option>' + forumselect + '</select></div>']],
];
</script>
...
<tr class="hover">
	...
	<td class="td25">
		<input type="hidden" name="id[]" value="<?php echo trim($row[id])?>" /><?php echo trim($row[id])?>
	</td>

	<td class="td25">
		<input type="text" name="department_code[]" value="<?php echo trim($row['department_code'])?>" />
	</td>
...
<script>
		...
		$("#ssd tr td").find("input[name^=id]").each(function(k,v){

			$(this).parent().html('<input type="hidden" name="id[]" value="'+$(this).val()+'">'+(k+1));
		});
