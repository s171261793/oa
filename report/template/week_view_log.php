     ...
     <tbody>
          ...
          if(count($infoWeek_plan_fall) > 0)
          {
            ...
            <tr style="text-align:center;vertical-align: middle;" id="1er">
              ...
              $htmlWeekPlan .='
                ...
                <td>
                  <input type="number" min="0" max="10" step="0.01" name="week_second_H[]" value="'.$val['week_second_H'].'" style="width:200px;height:30px;">
                </td>
      ...
      <tr style="text-align:center;vertical-align: middle;" id="zuihouTwo">
          ...
          <td style="text-align:center;vertical-align: middle;">
             <select name="opinion_two" disabled="disabled" style="width:290px;">
              <option value="0">--请选择--</option>
...
<script type="text/javascript">
  ...
  $("#sub").click(function(){
      ...
      $('input[name^=week_second_H]').each(function(k,v){
          ...
          if($(this).val() <= 0   ){ heightNumber++;}
          heightNumbers += parseFloat(v.value);
      });

      if( heightNumber){ alert('单个计划任务占比不能为0'); return false;}
      if( heightNumbers > 1 || heightNumbers < 1){ alert('所有的计划任务的个人提议占比总和等于1,单个计划任务占比不能为0'); return false;}
  ...
  function addInfoFirst(obj)
  {
    ...
    //是否有
      ...
      if(conn == "序号") conn =0
      conn++;
      var scripts = "<?=$infoWeek_order['description']?>"; //总结
      var scripts_new = "<?=$infoWeek_order['description']?>"; //新增任务总结
      var scripts_new = "<?=$infoWeek_order['description']?>"; //是否计划不周
  ...
  function addInfoSecond(obj)
  {
    ...
    if(conn == "序号") conn =0
      conn++;
    var scripts = "<?=$infoWeek_order['description']?>";
  ...
  $(function(){
    $("select[name=week_number],select[name=departmentida]").click(function(){
        alert("此处不可选择！");
        return false;
    });   
})
