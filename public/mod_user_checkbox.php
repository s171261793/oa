...
if ($do == 'list') {
	...
	for($i=0;$i<sizeof($values);$i++){
		if(trim($values[$i])!=''){
			$sql="SELECT a.name,phone,id,b.* FROM ".DB_TABLEPRE."user a,".DB_TABLEPRE."user_view b WHERE a.id=b.uid and a.id = '".$values[$i]."'";

			$blog  =  $db->query("set names utf8");	
	...
	$string =  substr($html, 0, -1).'|'.substr($phone, 0, -1).'|'.substr($id, 0, -1);
	echo $string;
}
//获取二级
